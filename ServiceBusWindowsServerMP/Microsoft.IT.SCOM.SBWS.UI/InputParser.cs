...
namespace Microsoft.IT.SCOM.SBWS.UI
{
    ...
    {
        private const string ClassNamePrefix = "Microsoft.IT.SCOM.SBWS.Namespace.";
        private const string DiscoveryNamePrefix = "Microsoft.IT.SCOM.SBWS.InitialDiscovery.";
        private const string AccountOverrideNamePrefix = "Microsoft.IT.SCOM.SBWS.RunAsProfile.Account.";
        ...
        private void GetDiscoveryConfig(ITemplateContext templateContext, ref TemplateInputConfig templateConfig)
        {
            ...
            foreach (ManagementPackDiscovery discovery in SDKHelper.GetFolderItems<ManagementPackDiscovery>(this, templateContext.OutputFolder))
            {
                if (discovery.Name.StartsWith(DiscoveryNamePrefix, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    templateConfig.SBWSNamespaceName = matchs[0].Groups[1].Value;
                    templateConfig.SBWSHostName = matchs[1].Groups[1].Value;
                    return;
                }
            }
